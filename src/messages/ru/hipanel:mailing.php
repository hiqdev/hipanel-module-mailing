<?php
/**
 * Mailing module for HiPanel
 *
 * @link      https://github.com/hiqdev/hipanel-module-mailing
 * @package   hipanel-module-mailing
 * @license   BSD-3-Clause
 * @copyright Copyright (c) 2017, HiQDev (http://hiqdev.com/)
 */

return [
    'Client' => 'Клиент',
    'Client state' => 'Состяние клиента',
    'Clients' => 'Клиенты',
    'Domain' => 'Домен',
    'Domains' => 'Домены',
    'Email' => 'Email',
    'Exclude unsubscribed' => 'Исключить отписавшихся',
    'Export mailing list' => 'Экспорт списка',
    'Has domain' => 'Имеет домен',
    'Has server' => 'Имеет сервер',
    'Does not have server' => 'Не имеет серверов',
    'Does not have domain' => 'Не имеет доменов',
    'Can have server' => 'Может иметь сервер',
    'Can have domain' => 'Может иметь домен',
    'Language' => 'Язык',
    'Mailing preparation' => 'Подготовка рассылки',
    'PDU' => 'PDU',
    'Rack' => 'Стойка',
    'Seller' => 'Селлер',
    'Sellers' => 'Селлеры',
    'Server' => 'Сервер',
    'Servers' => 'Серверы',
    'State' => 'Состояние',
    'Switch' => 'Свитч',
    'Type' => 'Тип',
    'Set filters to start the mailing preparation' => 'Установить фильтры для начала подготовки рассылки',
    'Not set' => 'Не выбран',
    'Languages' => 'Языки',
    'Include subclients' => 'Включить субклиентов',
    'We are redirecting you to the mailing service, please wait a bit' => 'Мы переадресовываем вас на сервис рассылки, ожидайте, пожалуйста',
    'Begin mailing' => 'Начать рассылку',
    'Domain zones' => 'Доменные зоны',
    'Server (partial match)' => 'Сервер (частичное совпадение)',
    'Client logins list (comma-, or space-separated)' => 'Список клиентов (разделитель – запятая или пробел)',
];
